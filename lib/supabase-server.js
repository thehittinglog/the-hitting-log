...
async function runAdminRequest(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    const requestError = new Error(
      responseBody?.message || `Supabase database request failed with HTTP ${response.status}`,
    );
    requestError.name = "SupabaseRequestError";
    requestError.code = responseBody?.code || "supabase_request_failed";
    requestError.details = responseBody?.details || null;
    requestError.hint = responseBody?.hint || null;
...
async function getSubscriptionBy(column, value) {
  ...
  const parameters = new URLSearchParams({
    select:
      "user_id,stripe_customer_id,stripe_subscription_id,subscription_status,stripe_price_id,current_period_start,current_period_end,cancel_at_period_end,plan,created_at,updated_at",
    [column]: `eq.${value}`,
    limit: "1",
  });
...
async function upsertSubscription(record) {
  ...
  try {
    await runAdminRequest(`${table}?${parameters.toString()}`, {
      method: "POST",
      headers: { Prefer: "resolution=merge-duplicates,return=minimal" },
      body: JSON.stringify(updatedRecord),
    });
  } catch (error) {
    error.subscriptionWriteFailed = true;
    console.error("SUBSCRIPTION_DATABASE_WRITE_ERROR", JSON.stringify({
      table,
      operation: "upsert",
      conflictTarget: "user_id",
      supabaseErrorCode: error?.code || null,
      supabaseErrorMessage: error?.message || "Unknown Supabase write error",
      httpStatus: error?.status || null,
      userId: record.user_id,
      subscriptionIdPresent: Boolean(record.stripe_subscription_id),
      normalizedTier: record.plan || null,
    }));
    throw error;
  }
}
