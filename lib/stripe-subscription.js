...
async function loadStripePriceCatalog(stripe, priceIds) {
  ...
  if (!proProductId || !proPlusProductId || proProductId === proPlusProductId) {
    const error = new Error("Configured Stripe membership prices must belong to distinct products.");
    error.code = "stripe_price_config_invalid";
    throw error;
  }
