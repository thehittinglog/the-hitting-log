...
function formatIndicatorEvidence(item) {
  ...
  if (item.key === "called_strikes") {
    return `${item.count} of ${item.denominator} recorded pitches were called strikes${percentage}.`;
  }
...
function formatPerformanceFallback(result) {
  ...
  if (result.intent === "performance_score_analysis" && result.biggestNegativeIndicator?.scoreImpact) {
    ...
    return [
      ...
      warning,
      result.biggestNegativeIndicator.recommendation,
    ].filter(Boolean).join("\n\n");
  }
  ...
  return [
    ...
    warning,
    primary.recommendation ? `For your next training session, ${primary.recommendation.charAt(0).toLowerCase()}${primary.recommendation.slice(1)}` : "",
  ].filter(Boolean).join("\n\n");
}
...
function formatDeterministicAnswer(result) {
  ...
  if (result.type === "split") {
    const prefix = result.dimension === "pitch velocity" ? `Against ${result.facts?.label}, your recorded result is:` : `For ${result.facts?.label}, your recorded result is:`;
    return [prefix, formatStatLine(result.facts), result.facts?.note, formatCoverageNote(result)].filter(Boolean).join("\n\n");
  }
  if (result.type === "metric") {
    ...
    const details = Object.entries(facts)
      ...
      .join("\n");
    return [`Your ${result.metric} is ${facts.percentage ?? "available below"}.`, details].filter(Boolean).join("\n\n");
  }
...
function isDirectStatisticalResult(result) {
  return new Set(["ranking", "split", "metric", "location", "contact_outcome_ranking", "overall", "recent", "recent_games", "trend"]).has(result?.type);
}
