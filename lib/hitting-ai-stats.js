...
function createInstructionalTopicResult(question, games) {
  ...
  const timingValues = atBats
    .map((atBat) => normalizeKey(atBat?.timing))
    .filter((timing) => ["early", "on_time", "late"].includes(timing));
  const timingCount = (timing) => timingValues.filter((value) => value === timing).length;
  const timing = {
    tracked: timingValues.length,
    early: timingCount("early"),
    onTime: timingCount("on_time"),
    late: timingCount("late"),
  };
  timing.earlyPercentage = timing.tracked ? formatPercent(timing.early / timing.tracked) : null;
  timing.onTimePercentage = timing.tracked ? formatPercent(timing.onTime / timing.tracked) : null;
  timing.latePercentage = timing.tracked ? formatPercent(timing.late / timing.tracked) : null;
  ...
  return {
    ...
    relevantData: {
      ...
      swingDecisions: {
        ...
        chases: outOfZonePitches.filter(isSwing).length,
        chaseRate: outOfZonePitches.length ? formatPercent(outOfZonePitches.filter(isSwing).length / outOfZonePitches.length) : null,
      },
      contact: {
        ballsInPlay: ballsInPlay.length,
        hardHits: ballsInPlay.filter((atBat) => atBat?.hardHitBall === true).length,
        hardHitPercentage: ballsInPlay.length ? formatPercent(ballsInPlay.filter((atBat) => atBat?.hardHitBall === true).length / ballsInPlay.length) : null,
      },
      twoStrikes: {
        plateAppearances: atBats.length,
        reached: twoStrikeAtBats.length,
        percentage: atBats.length ? formatPercent(twoStrikeAtBats.length / atBats.length) : null,
        strikeouts: twoStrikeAtBats.filter((atBat) => getOutcome(atBat) === "strikeout").length,
      },
    },
  };
}
