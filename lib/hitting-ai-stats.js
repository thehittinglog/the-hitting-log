...
} = require("../scripts/pitch-location-grid");
const canonicalStats = require("../scripts/non-proprietary-stats");
const { calculateHlpScore } = require("./hlp-score");

const HIT_OUTCOMES = canonicalStats.HIT_OUTCOMES;
const OFFICIAL_AT_BAT_OUTCOMES = canonicalStats.OFFICIAL_AT_BAT_OUTCOMES;
const OUT_OUTCOMES = canonicalStats.OUT_OUTCOMES;
...
const PERFORMANCE_SCORE_PATTERN = /\b(performance score|hlp score|hlp|score)\b/i;
...
const TRAINING_RECOMMENDATION_PATTERN = /\b(train(?:ing)?|practi[cs]e|work on|focus(?: on)?|priority|pay(?:ing)? attention|should (?:i )?fix|what (?:can|could|should) i fix)\b|\bhow (?:do|can|could|should) i (?:improve|get better|lower|raise|increase|decrease|fix|adjust|time up)\b|\bwhat can i do about\b|\bhow can i be better at\b|\bwhat (?:can|could|should|would) i improve\b|\bbetter hitter\b/i;
...
const HLP_PROPRIETARY_REQUEST_PATTERN = /(?:(?:\bhlp(?:\s+score)?\b|\b(?:hitting log performance|performance)\s+score\b).*\b(?:formula|calculat\w*|weights?|percent(?:age)?|contribut\w*|coefficients?|constants?|thresholds?|normaliz\w*|equation|algorithm|recreat\w*|code|internals?|methodology|scoring rule|exactly|change by|base64|encode|clue)\b)|(?:\b(?:formula|weight|percent(?:age)?|contribut\w*|coefficient|equation|algorithm|methodology|scoring rule)s?\b.*\b(?:hlp|performance score)\b)|(?:\b(?:what|show|give|tell|print|reveal)\b.*\b(?:weights?|coefficients?|equation|algorithm)\b)/i;
...
const HLP_NON_DISCLOSURE_MESSAGE = "The HLP Score is a proprietary performance metric developed by The Hitting Log. I can explain what your score represents and which areas of your performance are influencing it, but the underlying formula and weighting system are not publicly disclosed.";

function isHlpProprietaryRequest(question) {
  const value = String(question || "");
  return HLP_PROPRIETARY_REQUEST_PATTERN.test(value)
    || (INJECTION_PATTERN.test(value) && /\b(?:hlp|performance score|formula|weights?|coefficients?)\b/i.test(value));
}

// This is the authoritative language layer for intentionally public metrics.
// HLP scoring remains isolated in the server-only scoring module.
const METRIC_KNOWLEDGE_BASE = {
  performanceScore: {
    ...
    formula: "The underlying formula and weighting system are proprietary and are not publicly disclosed.",
    ...
    improvement: "Improve the lowest supported score component, then use decision, timing, and contact evidence to choose an approach focus.",
  },
  hardHitPercentage: {
    ...
    improvement: "Increase the share of balls in play marked hard hit by checking swing decisions first, timing second, and middle-of-the-ball contact intention third.",
  },
  twoStrikePercentage: {
    ...
    improvement: "More plate appearances must end before reaching two strikes; first evaluate takes on hittable early strikes and swings at pitches outside the zone.",
  },
  hardHitTwoStrikePercentage: {
    ...
    desirability: "Higher is generally better.",
    improvement: "Improve two-strike decisions first, timing second, and middle-of-the-ball contact intention third.",
  },
  ...
  chaseRate: {
    ...
    improvement: "Lower it by attacking strikes while refusing swings at pitches recorded outside the zone.",
  },
  contactRate: {
    ...
    improvement: "Check whether swings were at strikes, then timing, before changing contact intention.",
  },
  ...
  onTimePercentage: {
    ...
    improvement: "Keep the gather early and slow, then adjust front-foot landing relative to pitch speed.",
  },
  earlyPercentage: {
    ...
    improvement: "Keep starting the gather early and slowly, but let the move into front-foot landing develop longer.",
  },
  latePercentage: {
    ...
    improvement: "Keep the gather early and slow, but work toward getting the front foot down sooner.",
  },
...
const METRIC_KNOWLEDGE = Object.freeze(Object.fromEntries(
  Object.entries(METRIC_KNOWLEDGE_BASE).map(([key, metric]) => [key, Object.freeze({
    ...metric,
    improvement: metric.improvement || `Use the formula to identify which recorded inputs change this metric, then interpret that change in context. ${metric.desirability}`,
  })])
));

const INSTRUCTIONAL_TOPICS = Object.freeze({
  timing: {
    label: "timing",
    aliases: /\btiming\b|\bon time\b|\btime up\b|\b(?:i am|i'm|im) (?:late|early)\b|\b(?:always|often|a lot|frequently) (?:late|early)\b|\b(?:late|early) (?:a lot|often|frequently)\b|\bwhy am i (?:late|early)\b|\b(?:slow|slower|fast|faster) pitch(?:er|ing)?\b/i,
    definition: "Timing describes whether contact was recorded early, on time, or late relative to the pitch.",
  },
  swingDecision: {
    label: "swing decisions",
    aliases: /swing at better pitches|better pitch(?:es)? to hit|pitch selection|swing decisions?/i,
    definition: "Swing decisions describe whether the hitter attacks strikes and avoids expanding to pitches outside the recorded zone.",
  },
  contactQuality: {
    label: "contact quality",
    aliases: /weak contact|better contact|make (?:more )?solid contact|contact quality/i,
    definition: "Contact quality describes how well recorded balls in play were struck, using hard-hit and batted-ball information rather than inferred mechanics.",
  },
  twoStrikeApproach: {
    label: "two-strike performance",
    aliases: /(?:get|getting|keep getting|better|improve).*(?:two|2)[- ]strikes?|(?:two|2)[- ]strike (?:approach|performance)/i,
    definition: "Two-strike performance combines how often plate appearances reach two strikes with the decisions, outcomes, and contact recorded after they do.",
  },
});

function normalizeKey(value) {
  return canonicalStats.normalizeKey(value);
}

function normalizeOutcome(value, atBat = {}) {
  return canonicalStats.normalizeOutcome(value, atBat);
}

function getPitches(atBat) {
  return canonicalStats.getPitches(atBat);
}

function getOutcome(atBat) {
  return canonicalStats.getOutcome(atBat);
}
...
function getPitchResult(pitch) {
  return canonicalStats.getPitchResult(pitch);
}
...
function summarize(atBats) {
  return canonicalStats.calculateStatsFromAtBats(atBats);
}
...
function calculateRequestedMetric(atBats, metric, games = []) {
  const stats = canonicalStats.calculateMetrics(atBats);
  const gameStats = canonicalStats.calculateGameMetrics(games);
  ...
  if (metric.key === "hardHitPercentage") {
    return ratio(stats.hardHitBalls, stats.hardHitEligibleBalls);
  }
  if (metric.key === "twoStrikePercentage") return ratio(stats.twoStrikeAtBats, stats.completedPlateAppearances);
  if (metric.key === "hardHitTwoStrikePercentage") {
    return ratio(stats.twoStrikeHardHits, stats.twoStrikeBallsInPlay);
  }
  if (metric.key === "productiveOutPercentage") {
    return ratio(stats.productiveOuts, stats.totalOuts);
  }
  if (["lineDrivePercentage", "groundBallPercentage", "flyBallPercentage"].includes(metric.key)) {
    const values = {
      lineDrivePercentage: [stats.lineDrives, stats.lineDrivePercent],
      groundBallPercentage: [stats.groundBalls, stats.groundBallPercent],
      flyBallPercentage: [stats.flyBalls, stats.flyBallPercent],
    }[metric.key];
    return { rawValue: values[1], numerator: values[0], denominator: stats.ballsInPlay };
  }
  if (metric.key === "extraBaseHitPercentage") {
    return ratio(stats.extraBaseHits, stats.hits);
  }
  if (metric.key === "chaseRate") {
    return ratio(stats.outOfZoneSwings, stats.outOfZonePitches);
  }
  if (metric.key === "contactRate") {
    return ratio(stats.contactSwings, stats.swings);
  }
  if (metric.key === "qualityAtBatPercentage") return ratio(stats.qualityAtBats, stats.completedPlateAppearances);
  if (["onTimePercentage", "earlyPercentage", "latePercentage"].includes(metric.key)) {
    const count = { onTimePercentage: stats.onTime, earlyPercentage: stats.early, latePercentage: stats.late }[metric.key];
    return ratio(count, stats.timingTotal);
  }
  if (metric.key === "strikePercentage") {
    return { rawValue: stats.strikePercentage, numerator: null, denominator: null };
  }
  if (metric.key === "hitsPerGame") return ratio(gameStats.hits, gameStats.games);
  if (metric.key === "atBatsPerGame") return ratio(gameStats.atBats, gameStats.games);
  if (["multiHitGames", "zeroHitGames", "bestSingleGame", "gamesWithHit"].includes(metric.key)) {
    return { rawValue: gameStats[metric.key], numerator: null, denominator: null };
  }
  if (["recentThreeAverage", "recentThreeOps"].includes(metric.key)) {
    ...
    const recentStats = canonicalStats.calculateGameMetrics(recentGames);
    return { rawValue: metric.key === "recentThreeAverage" ? recentStats.battingAverage : recentStats.ops, numerator: null, denominator: null };
  }
  const source = games.length ? gameStats : stats;
  return {
    rawValue: source[metric.key],
    numerator: metric.key === "battingAverage" ? source.hits : null,
    denominator: metric.key === "battingAverage" ? source.atBats : null,
  };
}
...
function createFormulaResult(question) {
  ...
  if (metric.key === "performanceScore") {
    return { type: "hlp_proprietary", responseMode: "safe_refusal", answer: HLP_NON_DISCLOSURE_MESSAGE };
  }
  return {
    type: "formula",
...
function classifyQuestionAction(question) {
  const normalized = String(question || "").toLowerCase();
  if (STAT_FORMULA_PATTERN.test(normalized) || /how (?:is|do|does) .* track(?:ed|ing)?|what .* determine/.test(normalized)) return "metric_formula";
  if (TREND_ANALYSIS_PATTERN.test(normalized) || /has my .* (?:improved|changed)|am i getting better/.test(normalized)) return "trend_analysis";
  if (COMPARISON_PATTERN.test(normalized)) return "comparison";
  if (/\bwhy\b|\bhurt(?:ing)?\b.*\bscore\b|what (?:is|are) (?:driving|causing|affecting|hurting)|what (?:drives|affects|hurts)/.test(normalized)) return "metric_diagnosis";
  if (/\b(?:i am|i'm|im)\s+(?:(?:always|usually|often|a lot)\s+)?(?:late|early)\b/.test(normalized)) return "metric_diagnosis";
  if (/how (?:do|can|could|should) i (?:improve|get better|lower|raise|increase|decrease|fix|adjust|time up|make better|swing at better)|what (?:generally )?improves?|what can i do about|what should i (?:do|work on)|how can i be better at/.test(normalized)) return "metric_improvement";
  if (/what does my .* mean|what should .* (?:tell|telling) me|how should i interpret|is (?:my |this )?.*(?:good|bad|high|low)/.test(normalized)) return "metric_interpretation";
  if (/what does|what (?:is|are) (?!my\b)|what .* means?|define|explain (?:the )?/.test(normalized)) return "metric_definition";
  if (/what (?:is|are) my|how (?:often|many)|show me my/.test(normalized)) return "metric_lookup";
  return "general_performance_analysis";
}

function getMetricRequestMode(question) {
  const action = classifyQuestionAction(question);
  return {
    metric_formula: "formula",
    metric_definition: "definition",
    metric_interpretation: "interpretation",
    metric_improvement: "coaching",
    metric_diagnosis: "diagnosis",
    trend_analysis: "trend",
  }[action] || "value";
}

function getInstructionalTopic(question) {
  const entry = Object.entries(INSTRUCTIONAL_TOPICS).find(([, topic]) => topic.aliases.test(String(question || "")));
...
function createMetricKnowledgeResult(question, games, handedness) {
  ...
  if (mode === "definition") {
    return { type: "metric_definition", responseMode: "definition", metric: metric.label, metricKey: metric.key, definition: metric.definition, formula: metric.formula, desirability: metric.desirability };
  }
  if (mode === "trend") {
    const orderedAtBats = flattenAtBats((Array.isArray(games) ? games : []).slice().sort((left, right) => String(left.date || "").localeCompare(String(right.date || ""))))
      .filter(canonicalStats.isCompletedPlateAppearance);
    if (orderedAtBats.length < 2) {
      return { type: "metric_trend", responseMode: "comparison", metric: metric.label, metricKey: metric.key, available: false };
    }
    const midpoint = Math.floor(orderedAtBats.length / 2);
    const earlier = calculateRequestedMetric(orderedAtBats.slice(0, midpoint), metric);
    const recent = calculateRequestedMetric(orderedAtBats.slice(midpoint), metric);
    if (!Number.isFinite(earlier.rawValue) || !Number.isFinite(recent.rawValue)) {
      return { type: "metric_trend", responseMode: "comparison", metric: metric.label, metricKey: metric.key, available: false };
    }
    return {
      type: "metric_trend",
      ...
      metricKey: metric.key,
      available: true,
      earlierValue: formatRequestedMetricValue(metric, earlier.rawValue),
      recentValue: formatRequestedMetricValue(metric, recent.rawValue),
      direction: recent.rawValue > earlier.rawValue ? "higher" : recent.rawValue < earlier.rawValue ? "lower" : "unchanged",
    };
  }

  const lookup = createStatLookup(question, games);
  if (mode === "value") return lookup;
  const hasValue = lookup?.type === "stat_lookup";

  const coaching = analyzeRecentPerformance("What should I work on?", games, handedness);
  const performance = !hasValue || lookup.rawValue === null || metric.key !== "performanceScore"
    ? null
  ...
  const scoreInfluences = metric.key === "performanceScore"
    ? (performance?.influences || coaching?.scoreInfluences || [])
    : [];
  const rawValue = metric.key === "performanceScore" && Number.isFinite(coaching?.performanceScore)
    ? coaching.performanceScore
    : hasValue ? lookup.rawValue : null;
  return {
    ...
    metric: metric.label,
    metricKey: metric.key,
    value: Number.isFinite(rawValue) ? formatRequestedMetricValue(metric, rawValue) : null,
    rawValue,
    numerator: hasValue ? lookup.numerator : null,
    denominator: hasValue ? lookup.denominator : null,
    definition: metric.definition,
    formula: metric.formula,
    desirability: metric.desirability,
    ...
    scoreInfluences,
    biggestNegativeIndicator: coaching?.biggestNegativeIndicator || null,
    coachingDiagnostic: coaching?.coachingDiagnostic || null,
    sampleDescription: hasValue ? lookup.sampleDescription : "",
    relevantDataAvailable: hasValue,
    sampleSizeWarnings: coaching?.sampleSizeWarnings || [],
  };
}

function createInstructionalTopicResult(question, games) {
  const topic = getInstructionalTopic(question);
  if (!topic) return null;
  const action = classifyQuestionAction(question);
  const orderedGames = (Array.isArray(games) ? games : []).slice().sort((left, right) => String(left.date || "").localeCompare(String(right.date || "")));
  const atBats = flattenAtBats(orderedGames);
  const canonical = canonicalStats.calculateMetrics(atBats);
  const timingValues = atBats
    .filter(canonicalStats.isCompletedPlateAppearance)
    .map((atBat) => canonicalStats.normalizeTiming(atBat?.timing))
    .filter(Boolean);
  const timing = {
    tracked: canonical.timingTotal,
    early: canonical.early,
    onTime: canonical.onTime,
    late: canonical.late,
  };
  timing.earlyPercentage = timing.tracked ? formatPercent(canonical.earlyPercent) : null;
  timing.onTimePercentage = timing.tracked ? formatPercent(canonical.onTimePercent) : null;
  timing.latePercentage = timing.tracked ? formatPercent(canonical.latePercent) : null;
  if (timingValues.length >= 2) {
    const midpoint = Math.floor(timingValues.length / 2);
    const earlier = timingValues.slice(0, midpoint);
    const recent = timingValues.slice(midpoint);
    const onTimeRate = (values) => values.filter((value) => value === "on_time").length / values.length;
    timing.trend = {
      earlierOnTimePercentage: formatPercent(onTimeRate(earlier)),
      recentOnTimePercentage: formatPercent(onTimeRate(recent)),
      direction: onTimeRate(recent) > onTimeRate(earlier) ? "improved" : onTimeRate(recent) < onTimeRate(earlier) ? "declined" : "unchanged",
    };
  } else {
    timing.trend = null;
  }
  ...
  return {
    ...
    responseMode: action === "metric_definition" ? "definition" : action === "metric_diagnosis" ? "analysis" : "coaching",
    action,
    ...
    relevantData: {
      timing,
      swingDecisions: {
        outOfZonePitches: canonical.outOfZonePitches,
        chases: canonical.outOfZoneSwings,
        chaseRate: canonical.chaseRate === null ? null : formatPercent(canonical.chaseRate),
      },
      contact: {
        ballsInPlay: canonical.hardHitEligibleBalls,
        hardHits: canonical.hardHitBalls,
        hardHitPercentage: canonical.hardHitPercent === null ? null : formatPercent(canonical.hardHitPercent),
      },
      twoStrikes: {
        plateAppearances: canonical.completedPlateAppearances,
        reached: canonical.twoStrikeAtBats,
        percentage: canonical.twoStrikePercent === null ? null : formatPercent(canonical.twoStrikePercent),
        strikeouts: atBats.filter((atBat) => canonicalStats.isCompletedPlateAppearance(atBat) && reachedTwoStrikes(atBat) && getOutcome(atBat) === "strikeout").length,
      },
    },
  };
}
...
function getBattedBallType(atBat) {
  return canonicalStats.getBattedBallType(atBat);
}

function isBallInPlay(atBat) {
  return canonicalStats.hasBallInPlay(atBat);
}

function isSwing(pitch) {
  return canonicalStats.isSwing(pitch);
}

function isContact(pitch) {
  return canonicalStats.isContact(pitch);
}

function getPitchZoneStatus(pitch) {
  return canonicalStats.getPitchZoneStatus(pitch);
}

function isQualityAtBat(atBat) {
  return canonicalStats.isQualityAtBat(atBat);
}

function reachedTwoStrikes(atBat) {
  return canonicalStats.reachedTwoStrikes(atBat);
}

function calculatePerformanceScore(atBats) {
  const result = calculateHlpScore(atBats);
  return { ...result, components: [], metrics: {} };
}
...
function analyzeRecentPerformance(question, games, handedness = null) {
  ...
  const rawAtBats = scope.atBats;
  const atBats = rawAtBats.filter(canonicalStats.isCompletedPlateAppearance);
  ...
  // Preserve the proprietary HLP scorer's existing input path while all public
  // statistics below use the canonical completed-PA population.
  const performance = calculatePerformanceScore(rawAtBats);
  ...
  const asksAboutScore = /performance score|\bhlp\b|hurt.*score|improve.*score|lower.*score/.test(normalizedQuestion);
  ...
  const scoreInfluences = performance.influences || [];
  const biggestNegativeIndicator = asksAboutScore
...
function analyzeQuestion({ message, history = [], games = [], handedness = null }) {
  ...
  const normalized = question.toLowerCase();
  if (isHlpProprietaryRequest(question)) {
    return { type: "hlp_proprietary", responseMode: "safe_refusal", answer: HLP_NON_DISCLOSURE_MESSAGE };
  }
  const asksForLocation = /zone|pitch location|inside|outside|away|inner|outer|up and|down and|where/.test(normalized);
  const atBats = flattenAtBats(games).filter(canonicalStats.isCompletedPlateAppearance);
  ...
  const instructionalTopic = requestedMetric ? null : getInstructionalTopic(question);
  const metricMode = requestedMetric ? getMetricRequestMode(question) : null;
  if (requestedMetric && ["formula", "definition", "interpretation", "coaching", "diagnosis", "trend"].includes(metricMode)) {
    return createMetricKnowledgeResult(question, games, handedness);
  }
  if (instructionalTopic) return createInstructionalTopicResult(question, games);
  ...
  if (/hard[- ]?hit/.test(normalized)) {
    const metrics = canonicalStats.calculateMetrics(atBats);
    return metrics.hardHitEligibleBalls ? { type: "metric", metric: "hard-hit percentage", facts: { percentage: formatPercent(metrics.hardHitPercent), hardHitBalls: metrics.hardHitBalls, trackedBattedBalls: metrics.hardHitEligibleBalls } } : { type: "missing_data", field: "hard-hit results" };
  }

  if (/contact/.test(normalized)) {
    const metrics = canonicalStats.calculateMetrics(atBats);
    return metrics.swings ? { type: "metric", metric: "contact percentage", facts: { percentage: formatPercent(metrics.contactRate), contacts: metrics.contactSwings, swings: metrics.swings } } : { type: "missing_data", field: "swing and contact results" };
  }
...
function formatInstructionalGuidance(result) {
  const data = result.relevantData || {};
  if (result.topic === "timing") {
    const timing = data.timing || {};
    const timingLine = timing.tracked
      ? `Your log has ${timing.earlyPercentage} early, ${timing.onTimePercentage} on time, and ${timing.latePercentage} late across ${timing.tracked} timing-tracked plate appearance${timing.tracked === 1 ? "" : "s"}.`
      : "I don't have enough recorded timing data yet to tell whether you're primarily early, on time, or late.";
    if (result.action === "metric_definition") {
      return /on time/i.test(result.originalQuestion)
        ? `“On time” means your contact timing was recorded as synced with the pitch—not early or late. Timing is entered by the hitter for each tracked plate appearance, and the three classifications are early, on time, and late. ${timingLine}`
        : `${result.definition} It is based on the hitter's early, on-time, or late entry for each tracked plate appearance. ${timingLine}`;
    }
    if (result.action === "trend_analysis") {
      return timing.trend
        ? `Your recorded on-time rate has ${timing.trend.direction}, from ${timing.trend.earlierOnTimePercentage} in the earlier half of your timing entries to ${timing.trend.recentOnTimePercentage} in the recent half.`
        : `I need at least two recorded timing entries to evaluate whether your timing has improved.`;
    }
    const normalized = String(result.originalQuestion || "").toLowerCase();
    const slowSpecific = /slow|slower/.test(normalized);
    const fastSpecific = /fast|faster/.test(normalized);
    const describesLate = /\blate\b/.test(normalized);
    const describesEarly = /\bearly\b/.test(normalized);
    const first = slowSpecific
      ? "To time slower pitching, keep the gather early and slow, but let the move develop longer so the front foot lands somewhat after the pitch reaches halfway home."
      : fastSpecific
        ? "To time faster pitching, keep the gather early and slow, but get the front foot down somewhat before the pitch reaches halfway home."
        : result.action === "metric_diagnosis" && describesLate
          ? "The log cannot prove why you are late, but the supported adjustment is to keep the gather early and slow and work toward getting the front foot down sooner."
          : result.action === "metric_diagnosis" && describesEarly
            ? "The log cannot prove why you are early, but the supported adjustment is to keep the gather early and slow and let the move into front-foot landing develop longer."
            : "To improve timing, start the gather early and slow—when the pitcher starts moving, start moving—then adjust when the front foot lands without changing that early start.";
    const framework = "At a comfortable speed, the front foot should generally land around halfway home; against faster pitching, land somewhat before halfway, and against slower pitching, land somewhat after halfway.";
    let personalization = timingLine;
    if (timing.tracked) {
      if (timing.late > timing.early && timing.late > timing.onTime) {
        personalization += " Late is your largest recorded timing group, so getting the front foot down sooner is the supported timing focus; that pattern does not prove when your load began.";
      } else if (timing.early > timing.late && timing.early > timing.onTime) {
        personalization += " Early is your largest recorded timing group, so letting the move into landing develop longer is the supported timing focus; keep the gather itself early.";
      } else if (timing.onTime >= timing.early && timing.onTime >= timing.late) {
        personalization += " On time is already your largest recorded timing group, so timing may not be the first thing to change.";
      }
    }
    return [first, framework, personalization].join("\n\n");
  }

  if (result.topic === "swingDecision") {
    const decisions = data.swingDecisions || {};
    const personalized = decisions.chaseRate
      ? `Your recorded Chase Rate is ${decisions.chaseRate} (${decisions.chases} swings at ${decisions.outOfZonePitches} out-of-zone pitches).`
      : "I don't have enough recorded pitch-location and swing data to calculate your Chase Rate yet.";
    if (result.action === "metric_definition") return `${result.definition} ${personalized}`;
    return [`To swing at better pitches, look to attack strikes without expanding to pitches outside the recorded zone.`, personalized].join("\n\n");
  }

  if (result.topic === "contactQuality") {
    const contact = data.contact || {};
    const personalized = contact.hardHitPercentage
      ? `Your recorded Hard Hit Ball % is ${contact.hardHitPercentage} across ${contact.ballsInPlay} eligible balls in play.`
      : "I don't have enough eligible balls in play to personalize this with Hard Hit Ball % yet.";
    if (result.action === "metric_definition") return `${result.definition} ${personalized}`;
    return [
      "To improve contact quality, check the three questions in order: did you swing at a strike, were you on time, and did you hit the middle of the ball?",
      "If the decision and timing were sound, use a middle-of-the-baseball, line-drive intention; the log does not support prescribing a mechanical change.",
      personalized,
    ].join("\n\n");
  }

  if (result.topic === "twoStrikeApproach") {
    const twoStrikes = data.twoStrikes || {};
    const personalized = twoStrikes.percentage
      ? `You reached two strikes in ${twoStrikes.percentage} of recorded plate appearances, with ${twoStrikes.strikeouts} strikeout${twoStrikes.strikeouts === 1 ? "" : "s"} in those situations.`
      : "I don't have recorded plate appearances yet to personalize the two-strike guidance.";
    if (result.action === "metric_definition") return `${result.definition} ${personalized}`;
    return [
      "To get better with two strikes, start with pitch decisions: protect against strikes without expanding to balls, then check timing, then use a middle-of-the-ball contact intention.",
      personalized,
    ].join("\n\n");
  }
...
function formatDeterministicAnswer(result) {
  ...
  if (result.type === "hlp_proprietary") return HLP_NON_DISCLOSURE_MESSAGE;
  ...
  if (result.type === "metric_definition") {
    ...
    return `${name} is ${result.definition}. It is calculated as ${result.formula}. ${result.desirability}`;
  }
  if (result.type === "instructional_guidance") return formatInstructionalGuidance(result);
  if (result.type === "metric_trend") {
    return result.available
      ? `Your recent ${result.metric} is ${result.recentValue}, compared with ${result.earlierValue} in the earlier half of the tracked sample. It is ${result.direction}.`
      : `I don't have enough relevant recorded data to evaluate a trend in your ${result.metric}.`;
  }
  if (result.type === "metric_guidance") {
    const name = result.metric === "OPS" ? "OPS" : result.metric;
    const opening = result.value ? `Your ${name} is ${result.value}.` : `I don't have enough relevant recorded data to calculate your ${name} yet.`;
    const diagnostic = result.coachingDiagnostic;
    const evidence = diagnostic?.indicator ? formatIndicatorEvidence(diagnostic.indicator) : "";
    const warning = result.sampleSizeWarnings?.[0] || "";

    if (result.requestMode === "interpretation") {
      return [opening, `It measures ${result.definition}.`, result.desirability].filter(Boolean).join("\n\n");
    }
    ...
    if (result.requestMode === "diagnosis") {
      ...
      if (result.metricKey === "performanceScore") {
        return [opening, "It summarizes your recorded offensive process. I can discuss high-level performance areas, but not the proprietary scoring mechanics.", warning].filter(Boolean).join("\n\n");
      }
    ...
    if (result.metricKey === "twoStrikePercentage") {
      ...
      adjustment = `${adjustment} I can connect that advice to high-level HLP performance areas, but not disclose the proprietary weighting system.`;
    ...
    let metricChange = result.improvement;
    if (result.metricKey === "twoStrikePercentage") {
      metricChange = "Two-strike percentage rises whenever a plate appearance reaches two strikes. To lower it, more plate appearances need to end before reaching two strikes.";
    } else if (result.metricKey === "hardHitPercentage") {
      metricChange = "Hard Hit Ball % rises when a greater share of eligible balls in play are recorded as hard hit.";
    } else if (result.metricKey === "chaseRate") {
      metricChange = "Chase Rate falls when fewer recorded out-of-zone pitches result in swings.";
    }
    return [opening, metricChange, adjustment, warning].filter(Boolean).join("\n\n");
  }
...
function isDirectStatisticalResult(result) {
  return new Set(["hlp_proprietary", "stat_lookup", "formula", "metric_definition", "metric_guidance", "metric_trend", "instructional_guidance", "comparison", "ranking", "split", "metric", "location", "contact_outcome_ranking", "overall", "recent", "recent_games", "trend"]).has(result?.type);
}
