...
function getContextQuestion(message, history) {
  ...
  if (!/^(what about|how about|and |what if|was that|is that|did that|has that|does that|\d{1,3}\s*(?:-|to)\s*\d{1,3})/i.test(trimmed)) return trimmed;
...
function selectRecentSample(question, games) {
  ...
  const prefersTournament = explicitTournament || /this week|work on|focus|weakness|doing well|hurt|improve|caus.*outs|current|lately|recent trend/.test(normalized);
...
function analyzeQuestion({ message, history = [], games = [] }) {
  ...
  if (/work on|focus|workout|hurt.*(?:weekend|score|performance)|lower.*score|improve.*score|performance score|biggest weakness|doing well|how am i doing|strength|caus.*most.*outs|caused most.*outs|what trend|paying attention|was that just|is that just/.test(normalized)) {
