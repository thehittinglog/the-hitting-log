...
function classifyQuestionAction(question) {
  ...
  if (/how (?:do|can|could|should) i (?:improve|get better|lower|raise|increase|decrease|fix|adjust|time up|make better|swing at better)|what can i do about|what should i (?:do|work on)|how can i be better at/.test(normalized)) return "metric_improvement";
...
function isDirectStatisticalResult(result) {
  return new Set(["stat_lookup", "formula", "metric_definition", "metric_guidance", "metric_trend", "instructional_guidance", "comparison", "ranking", "split", "metric", "location", "contact_outcome_ranking", "overall", "recent", "recent_games", "trend"]).has(result?.type);
}
