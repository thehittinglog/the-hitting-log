...
async function findRelevantStripeSubscription(stripe, customerIds, priceIds, catalog = {}) {
  ...
  for (const customerId of customerIds) {
    ...
    const subscription = selectRelevantSubscription(subscriptions.data, priceIds, catalog);
...
async function reconcileSubscription(stripe, user, localSubscription, priceIds, catalog = {}) {
  ...
  if (!stripeSubscription) {
    ...
    return { customerFound, subscription: freeRecord, changed, normalization: null };
  }
  ...
  return { customerFound, subscription: record, changed, normalization };
}
