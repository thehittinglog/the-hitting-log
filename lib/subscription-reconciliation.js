...
async function findRelevantStripeSubscription(stripe, customerIds, priceIds) {
  ...
  for (const customerId of customerIds) {
    ...
    if (subscription) return { customerFound, subscription };
  }
...
async function reconcileSubscription(stripe, user, localSubscription, priceIds) {
  ...
  if (localCustomerIds.length) {
    const result = await findRelevantStripeSubscription(stripe, localCustomerIds, priceIds);
  ...
  if (!stripeSubscription || !RECONCILABLE_SUBSCRIPTION_STATUSES.includes(stripeSubscription.status)) {
    ...
    const result = await findRelevantStripeSubscription(stripe, emailCustomerIds, priceIds);
    customerFound = customerFound || result.customerFound;
  ...
  if (!stripeSubscription || !RECONCILABLE_SUBSCRIPTION_STATUSES.includes(stripeSubscription.status)) {
    ...
    const result = await findRelevantStripeSubscription(stripe, sessionCustomerIds, priceIds);
    customerFound = customerFound || result.customerFound;
    stripeSubscription = selectRelevantSubscription(
      [stripeSubscription, result.subscription].filter(Boolean),
      priceIds,
  ...
  if (!stripeSubscription) {
    ...
    return { customerFound, subscription: freeRecord, changed };
  }

  const record = toSubscriptionRecord(stripeSubscription, user.id, priceIds);
  ...
  return { customerFound, subscription: record, changed };
}
