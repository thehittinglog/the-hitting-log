(function (root, factory) {
  ...
  function getAiEligibility({ dateOfBirth, guardianPermissionConfirmedAt } = {}, referenceDate = new Date()) {
    const age = calculateAge(dateOfBirth, referenceDate);
    if (age === null) return { eligible: false, code: "date_of_birth_required", age: null };
    if (age < 13) return { eligible: false, code: "ai_age_restricted", age };
  ...
  return { calculateAge, getAiEligibility, normalizeDateOfBirth };
});
